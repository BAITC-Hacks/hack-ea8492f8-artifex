...
RU = {
    ...
    "unit_created": ("созданное подразделение", "созданных подразделения",
                     "созданных подразделений"),
    "unit_abolished": ("упразднённое подразделение", "упразднённых подразделения",
                       "упразднённых подразделений"),
    "unit_transformed": ("преобразованное подразделение", "преобразованных подразделения",
                         "преобразованных подразделений"),
}
...
def summarize(findings, before, after, doc_titles):
    ...
    return {
        ...
        "by_type": [
            {"type": t, "label": plural(len(v), *RU.get(t, (t, t, t))),
             "count": len(v),
...
CONSEQUENCE = {
    "unit_abolished": ("structure", "Структура изменилась"),
    "unit_created": ("structure", "Структура изменилась"),
    "unit_transformed": ("structure", "Структура изменилась"),
...
CONSEQUENCE_ORDER = ["not_done", "blurred", "contradiction", "softened",
                     "structure", "tracked"]
CONSEQUENCE_HINT = {
    ...
    "structure": "Какие подразделения упразднены, преобразованы и созданы. "
                 "Сопоставление ведётся по функционалу и составу, а не по "
                 "наименованию: департамент переименовывают чаще, чем переделывают.",
}

RECOMMEND = {
    "unit_abolished": "Проверить, что все функции упразднённого подразделения "
                      "переданы явно, а не растворились.",
    "unit_created": "Убедиться, что функционал нового подразделения не дублирует "
                    "существующие.",
    "unit_transformed": "Сверить, какие функции остались за подразделением, "
                        "а какие ушли вместе с должностями.",
...
ASSERTS_ABSENCE = {"orphan", "lost", "unit_abolished"}
